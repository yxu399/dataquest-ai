...
async def send_chat_message(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return ChatResponse(message=error_message, error=str(e))


@router.get("/conversation/{analysis_id}")
async def get_conversation_history(analysis_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "conversation": [],
        "data_summary": {
            "shape": analysis.data_profile.get("shape")
            if analysis.data_profile
            else None,
            "columns": analysis.data_profile.get("columns")
            if analysis.data_profile
            else None,
