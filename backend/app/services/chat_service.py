...
class ChatService:
    ...
    async def _process_with_ai(
        self, user_message: str, context: Dict, conversation_history: List = None
    ) -> Dict[str, Any]:
    ...
    def _process_with_fallback(
        ...
        if column_mentions:
            return self._handle_column_specific_query(
                user_message, column_mentions, context
            )

        # Key insights request
        if any(
            word in message_lower for word in ["insight", "key", "finding", "summary"]
        ):
            ...
            if insights:
                ...
                if corr_count > 0:
                    insight_text += (
                        f"• Found {corr_count} strong correlations between variables\n"
                    )

            return {"content": insight_text.strip(), "chart_type": None}

        # Correlation requests
        if any(
            word in message_lower for word in ["correlation", "relationship", "heatmap"]
        ):
            corr_count = len(analysis_results.get("correlations", []))
            if corr_count > 0:
                ...
                return {"content": content, "chart_type": "correlation"}
            else:
                return {
        ...
        # Bar chart requests
        if (
            any(word in message_lower for word in ["bar", "categor", "distribution"])
            and "numeric" not in message_lower
        ):
        ...
        if any(
            word in message_lower for word in ["scatter", "relationship", "numeric"]
        ):
        ...
        # Histogram requests
        if any(
            word in message_lower for word in ["histogram", "distribution", "spread"]
        ):
            num_cols = len(data_profile.get("numeric_columns", []))
            if num_cols > 0:
                return {
        ...
        # Box plot requests
        if any(
            word in message_lower for word in ["box", "boxplot", "summary", "quartile"]
        ):
            num_cols = len(data_profile.get("numeric_columns", []))
            if num_cols > 0:
                return {
...
Provide specific, data-driven answers based on the actual data provided above."""
    ...
    def _parse_claude_response(
        self, claude_response: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _get_chart_data(
        self, chart_type: str, context: Dict[str, Any]
    ) -> Optional[Dict]:
    ...
    def _extract_column_mentions(
        self, user_message: str, data_profile: Dict
    ) -> List[str]:
    ...
    def _handle_column_specific_query(
        self, user_message: str, columns: List[str], context: Dict
    ) -> Dict[str, Any]:
        ...
        if any(
            ...
            categorical_cols = [
                col
                for col in columns
                if col in data_profile.get("categorical_columns", [])
            ]
            numeric_cols = [
                col for col in columns if col in data_profile.get("numeric_columns", [])
            ]
        ...
        if len(columns) == 1:
            ...
            col_type = (
                "numeric"
                if col in data_profile.get("numeric_columns", [])
                else "categorical"
            )
            return {
