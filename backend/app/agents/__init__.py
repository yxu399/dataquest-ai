...
def data_profiler_agent(state: DataAnalysisState) -> DataAnalysisState:
    ...
    try:
        ...
        profile = {
            ...
            "full_data": df.head(1000).to_dict("records")
            if len(df) <= 1000
            else df.sample(1000).to_dict("records"),
        }
        ...
        return {**state, "data_profile": profile, "status": "profiled", "error": None}
    ...
    try:
        ...
        if profile:
            ...
            if missing_pct > 10:
                ...
                insights.append(
                    f"✓ Low missing data: {missing_pct:.1f}% of values are missing"
                )
            else:
        ...
        if correlations:
            for corr in correlations[:3]:  # Top 3 correlations
                ...
                insights.append(
                    f"📊 {corr['column1']} and {corr['column2']} are strongly {direction} correlated ({corr['correlation']})"
                )
        ...
        if profile:
            ...
            if numeric_cols > categorical_cols:
                insights.append(
                    f"📈 Numeric-heavy dataset: {numeric_cols} numeric vs {categorical_cols} categorical columns"
                )
            elif categorical_cols > numeric_cols:
                insights.append(
                    f"📋 Category-heavy dataset: {categorical_cols} categorical vs {numeric_cols} numeric columns"
                )
            else:
                insights.append(
                    f"⚖️ Balanced dataset: {numeric_cols} numeric and {categorical_cols} categorical columns"
                )

        return {**state, "insights": insights, "status": "completed", "error": None}
